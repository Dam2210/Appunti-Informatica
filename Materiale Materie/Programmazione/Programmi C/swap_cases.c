...
void main()
{
    ...
    free(b); /* libero la memoria allocata a b */
    b = slice(a, 9, 9);
    if(b != NULL)
    {
        printf("%s\n", b);
...
/* Scrivere una funzione denominata slice che prende una stringa a e due
 * interi positivi i e j (si può assumere che i<j) e restituisce
 * una stringa composta dai caratteri a[i], a[i+1], ..... a[i-1].
...
char *slice(char *a, int i, int j)
{
    int n, h = strlen(a); /* tramite la libreria <string.h> viene implementata la funzione che restituisce la lunghezza di una stringa */
