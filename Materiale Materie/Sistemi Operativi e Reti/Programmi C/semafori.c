//Sei thread, uno scrittore e 5 lettori. 
...
void *funzioneS()
{
    sem_wait(&semaphoreScrittore); //blocco del semaforo sullo scrittore
    ...
    for(int i = 0; i < BUFFER_SIZE; i++)
    {
        if (i % 2 == 0)
        {
            ...
            //scrivo il numero dispari nella posizione i pari
            buffer[i] = num;
        ...
        else
        {
            if(i % 2 == 1)
            {
                ...
                //scrivo il numero pari nella posizione i dispari
                buffer[i] = num;
                printf("[Scrittore] - Ho inserito il numero %d in posizione %d\n", num, i);
            }
        }
...
int main()
{
    ...
    pthread_create(&scrittore, NULL, funzioneS, NULL);

    //Creo i thread lettori
    for (int i = 0; i < 5; i++) {
        pthread_create(&lettori[i], NULL, funzioneL, (void *)i);
    }
