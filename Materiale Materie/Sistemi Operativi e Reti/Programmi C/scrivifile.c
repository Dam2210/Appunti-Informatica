...
int main()
{
    ...
    if(p1 == 0)
    {
        ...
        printf("[Figlio1] --> apro il file in scrittura\n");
        ...
        while(count < 9)
        {
            ...
            {
                char buffer[10];
                int length = sprintf(buffer, "%d", num); // Converto il numero in stringa e memorizzo la lunghezza della stringa

                write(fd1, &buffer, length); //scrivo il numero nel file (il numero è contenuto nel buffer e la lunghezza è length)
        ...
        write(pipeFiglio[WRITE], &charcount, sizeof(charcount));
    }
    else
    {
        //codice padre
        ...
        if(p2 == 0)
        {
            ...
            int ok = 1;
            write(pipePadre[WRITE],&ok,sizeof(ok));
        }
        else
        {
